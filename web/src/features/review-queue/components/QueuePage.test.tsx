...
describe('QueuePage', () => {
  it('defaults to grouped incidents and confirms before accepting all records', async () => {
    stubQueueApi(
    ...
    // The destructive breadth is explicit before any decisions are sent.
    expect(screen.queryByRole('checkbox')).not.toBeInTheDocument();
    await userEvent.click(screen.getByRole('button', { name: 'Accept all' }));
    expect(screen.getByRole('dialog', { name: 'Accept all records' })).toBeInTheDocument();
    expect(screen.getByText(/accept all 4 records currently awaiting review/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Accept 4 records' })).toBeInTheDocument();
    await userEvent.click(screen.getByRole('button', { name: 'Cancel' }));
    expect(screen.queryByRole('dialog', { name: 'Accept all records' })).not.toBeInTheDocument();

    await userEvent.click(screen.getByRole('button', { name: 'Accept all' }));
    await userEvent.click(screen.getByRole('button', { name: 'Accept 4 records' }));
    await waitFor(() => {
  ...
  it('switches to the flat view with camera, zone, rule and status chips', async () => {
    stubQueueApi(
      makeQueuePage(
        [
          makeQueueEvent(),
    ...
    // No ambiguous selection controls are introduced; Accept all is explicit.
    expect(screen.queryByRole('checkbox')).not.toBeInTheDocument();
      ...
      makeIncidentsResponse([], 0, [
        {
          ...
          why_not_review: ["rule 'Mobile uses' watches 'cell phone'; frame classes: person, chair"],
          matched: false,
        },
      ]),
    );
