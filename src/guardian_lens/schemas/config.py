...
class CameraDiscoveryCandidateResponse(BaseModel):
    """A discovered camera awaiting user selection and import."""
    ...
    rtsp_paths: list[str]
    default_rtsp_path: str | None
    resolution: str | None
    codec: str | None
    status: str  # verified, unreachable, error
...
class DiscoveryScanResponse(BaseModel):
    """Status of a camera discovery scan operation."""
...
class CameraDiscoveryImportRequest(BaseModel):
